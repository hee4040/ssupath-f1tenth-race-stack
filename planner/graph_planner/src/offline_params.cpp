...
OfflineParams load_offline_params(rclcpp::Node * node) {
    ...
    // ---- offline ----
    p.lat_resolution = node->declare_parameter<double>("lat_resolution", 0.5);
    ...
    p.vel_max = node->declare_parameter<double>("vel_max", 9.0);
    ...
    p.w_inner = node->declare_parameter<double>("w_inner", 0.7);
    p.w_outer = node->declare_parameter<double>("w_outer", 1.3);
    ...
    p.ax_max        = node->declare_parameter<double>("ax_max", 3.8);
    p.ay_max        = node->declare_parameter<double>("ay_max", 5.4);
    p.drag_coeff    = node->declare_parameter<double>("drag_coeff", 0.25);
    p.m_veh         = node->declare_parameter<double>("m_veh", 6.0);
    p.dyn_model_exp = node->declare_parameter<double>("dyn_model_exp", 1.5);
    p.mu            = node->declare_parameter<double>("mu", 1.0);
    p.closed_track  = node->declare_parameter<bool>("closed_track", false);
