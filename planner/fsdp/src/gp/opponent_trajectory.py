...
from f110_msgs.msg import ObstacleArray, OpponentTrajectory, Obstacle, OppWpnt, WpntArray
from fsdp.msg import ProjOppTraj, ProjOppPoint
...
class Opponent_Trajectory:
    def __init__(self):
        ...
        rospy.Subscriber('/car_state/frenet/odom', Odometry, self.car_state_frenet_cb) # car frenet coordinates
    ...
    def state_machine_cb(self, data: String):
        # ROS2 state machine 은 "StateType.OVERTAKE" 형태로 발행한다
        if data.data.split(".")[-1] == "OVERTAKE":
            self.overtake = True
        else:
            self.overtake = False
    ...
    def make_opponent_trajectory(self):
        ...
        while not rospy.is_shutdown(): 
            ...
            # 이 줄이 `if opponent is not None:` 안에 있으면 상대차가 한 번도 안 잡힌
            # 상태에서 콜백이 영원히 돌지 않아(obstacle_array 갱신 안 됨) 데드락이 된다.
            # 반드시 while 루프 본문 레벨에 있어야 한다.
            rate.sleep()
...
#########################HELPER FUNCTIONS######################################
        ...
        if len(obstacle_array.obstacles) > 0 and len(position_in_map_frenet):
            ...
            for obstacle in obstacle_array.obstacles: 
                ...
                if opponent_dist <= closest_opp and not obstacle.is_static: 
                    ...
                    # 이 스택의 f110_msgs/Obstacle 에는 분산 필드가 없다(원래 ROS1 스택에만 있었다).
                    # 값은 fsdp 자체 메시지(ProjOppPoint)로 그대로 넘어가기만 하고 GP 계산에는
                    # 쓰이지 않으므로, 없으면 0 으로 둔다.
                    # (없을 때 AttributeError 로 이 노드가 죽으면 /opponent_trajectory 가 안 나와
                    #  GP 상대궤적 예측 체인 전체가 멈춘다)
                    s_var = getattr(obstacle, "s_var", 0.0)
                    d_var = getattr(obstacle, "d_var", 0.0)
                    vs_var = getattr(obstacle, "vs_var", 0.0)
                    vd_var = getattr(obstacle, "vd_var", 0.0)
    ...
    def project_opponent_trajectory(self, old_index:int, index:int, raw_opponent_traj: np.ndarray, proj_opponent_traj: ProjOppTraj, trav_dist: float, track_length: float, first_point: bool = False, bound_left: list = None, bound_right: list = None) -> (float, np.ndarray, bool):
        ...
        projected_opponent.vs = 0.0
        projected_opponent.vd = 0.0
