...
public:
  // 클러스터 한 개의 판정 결과. PASS* 는 통과, 나머지는 기각 사유다.
  enum class RejReason : size_t {
    PASS = 0,       // 문턱 안쪽으로 걸침
    PASS_NARROW,    // |d| <= smallest_d -> 레이싱라인 정중앙 예외
    FAR_S,          // max_viewing_distance 초과
    OUT_WIDEST,     // 트랙 최광부 밖
    RIGHT_THR,      // 오른쪽 문턱선 밖
    LEFT_THR,       // 왼쪽 문턱선 밖
    COUNT
  ...
  : rclcpp::Node("cluster_to_obstacle"),
    ...
    // 경계 판정에 쓸 클러스터 반폭의 비율 [-]. 0.5 = 실제 반폭(= 이 값 도입 전 거동).
    // 낮출수록 장애물을 좁게 보므로 문턱을 넘기 어려워진다(벽 유령 억제).
    // 0.5 를 넘으면 실제보다 부풀려 보게 되어 문턱의 의미가 사라지므로 상한을 둔다.
    // 기각 사유별 카운터를 이 주기[s]마다 한 줄로 찍는다. 0 이하면 통계 자체를 끈다.
    // 카운터는 정수 증가뿐이고 로그는 주기당 1줄이라 주행 부하는 사실상 없다.
    stats_log_period_       = declare_parameter<double>("stats_log_period", 5.0);
    edge_fraction_          = declare_parameter<double>("edge_fraction", 0.5);
    {
      const double raw = edge_fraction_;
      edge_fraction_ = std::clamp(edge_fraction_, 0.0, 0.5);
      if (raw != edge_fraction_) {
        RCLCPP_WARN(get_logger(),
          "[cluster_to_obstacle] edge_fraction %.3f -> %.3f 로 클램프됨 (허용 범위 [0.0, 0.5])",
          raw, edge_fraction_);
      }
    }
    ...
    RCLCPP_INFO(get_logger(), "[cluster_to_obstacle] min_intrusion=%.3f m, edge_fraction=%.3f, stats_log_period=%.1f s",
                min_intrusion_, edge_fraction_, stats_log_period_);
    stats_last_log_ = now();
  }
...
private:
  ...
  void clusterCb(const tier4_perception_msgs::msg::DetectedObjectsWithFeature::SharedPtr msg) {
    ...
    stat_msgs_++;
    stat_clusters_ += msg->feature_objects.size();
    ...
    stat_published_ += published;
    maybeLogStats();
  }

  // 기각 사유별 카운터를 주기적으로 한 줄 요약한다.
  // "노란 점이 빨간 선 안인데 왜 안 잡히나" 를 추측 없이 가르는 것이 목적이다:
  //   문턱(우/좌문턱, 최광부밖) 때문이면 min_intrusion / edge_fraction 문제,
  //   tf_fallback/tf_fail 이 크면 위치 변환 문제라 문턱을 아무리 만져도 안 고쳐진다.
  void maybeLogStats() {
    if (stats_log_period_ <= 0.0) return;
    const rclcpp::Time t = now();
    if ((t - stats_last_log_).seconds() < stats_log_period_) return;
    stats_last_log_ = t;

    const uint64_t pass = stat_reason_[static_cast<size_t>(RejReason::PASS)]
                        + stat_reason_[static_cast<size_t>(RejReason::PASS_NARROW)];
    const double rate = stat_clusters_ ? 100.0 * static_cast<double>(pass) / static_cast<double>(stat_clusters_) : 0.0;
    RCLCPP_INFO(get_logger(),
      "[c2o] %lu msg / %lu cluster -> %lu obs (%.1f%%) | tf(fallback %lu, fail %lu) | "
      "기각: 거리 %lu, 최광부밖 %lu, 우문턱 %lu, 좌문턱 %lu | 통과: 일반 %lu, 최협부예외 %lu | drop(conv %lu, bound %lu)",
      stat_msgs_, stat_clusters_, stat_published_, rate,
      stat_tf_fallback_, stat_tf_fail_,
      stat_reason_[static_cast<size_t>(RejReason::FAR_S)],
      stat_reason_[static_cast<size_t>(RejReason::OUT_WIDEST)],
      stat_reason_[static_cast<size_t>(RejReason::RIGHT_THR)],
      stat_reason_[static_cast<size_t>(RejReason::LEFT_THR)],
      stat_reason_[static_cast<size_t>(RejReason::PASS)],
      stat_reason_[static_cast<size_t>(RejReason::PASS_NARROW)],
      stat_drop_noconv_, stat_drop_nobound_);

    // 구간 통계로 쓰기 위해 매번 리셋한다(누적이면 초반 값에 묻힌다).
    stat_msgs_ = stat_clusters_ = stat_published_ = 0;
    stat_tf_fallback_ = stat_tf_fail_ = stat_drop_noconv_ = stat_drop_nobound_ = 0;
    stat_reason_.fill(0);
  }
  ...
  RejReason classifyPoint(double s, double d, double car_s, double size = 0.0) const {
    ...
    if (d_hi <= -d_right_array_[idx]) return RejReason::RIGHT_THR;
    if (d_lo >=  d_left_array_[idx])  return RejReason::LEFT_THR;
...
private:
  ...
  double edge_fraction_{0.5};   // 경계 판정에 쓸 클러스터 반폭 비율 (0.5 = 실제 반폭)

  // ---- 기각 사유 통계 ----
  double stats_log_period_{5.0};
  rclcpp::Time stats_last_log_;
  std::array<uint64_t, static_cast<size_t>(RejReason::COUNT)> stat_reason_{};
  uint64_t stat_msgs_{0}, stat_clusters_{0}, stat_published_{0};
  uint64_t stat_tf_fallback_{0}, stat_tf_fail_{0};
  uint64_t stat_drop_noconv_{0}, stat_drop_nobound_{0};
