...
{
  const std::lock_guard<std::mutex> lock(mutex_ppp_ekf_); //srkang 250820
...
{
  const std::lock_guard<std::mutex> lock(mutex_ppp_ekf_);

  if (!is_activated_) return;
  if (current_ekf_pose_.header.frame_id.empty()) return;

  // child frame 이름 선택 (여러 EKF를 동시에 띄울 때 충돌 방지)
  std::string child_frame = "base_link";
  // ekf_mode_를 이미 쓰고 있으니, 모드별 child를 다르게 하고 싶다면:
  // if (ekf_mode_ == "lidar") {
  //   child_frame = "base_link_lidar";
  // } else if (ekf_mode_ == "camera" || ekf_mode_ == "gnss") {
  //   child_frame = "base_link_camera"; // 필요에 맞게 바꾸세요
  // }
  ...
  geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr initialpose)
{
  ...
  is_activated_ = true;
  RCLCPP_INFO(get_logger(), "[EKF] Initialized and Activated by /initialpose topic.");
}
...
void EKFLocalizer::publishEstimateResult()
{
  ...
  if (!pose_queue_.empty()) {
    ...
    pub_measured_pose_ = create_publisher<geometry_msgs::msg::PoseStamped>(
    "ekf_measured_pose", 1);  
  }
  ...
  pub_debug_ = create_publisher<tier4_debug_msgs::msg::Float64MultiArrayStamped>(
    "ekf_debug", 1);  
}
