...
private:
    ...
    int vgicp_k_correspondences_ {20};
    std::string vgicp_nn_method_ {"BRUTEFORCE"};
    double vgicp_kernel_width_ {0.5};

    bool   enable_angle_filter_{true}; // 전방 FOV 필터 on/off
    double fov_deg_{270.0};            // 전방 270도
    double half_fov_rad_{0.0};         // 생성자에서 계산해 채움

    // 바닥/천장 제거용 z 밴드. 수평면은 x/y/yaw를 전혀 구속하지 못하면서
    // 점 수로 정합 비용을 지배해 종방향 관측성을 죽인다.
    bool   enable_z_filter_{false};
    double scan_min_z_{0.2};
    double scan_max_z_{2.2};
    ...
    // previous_odom_mat_ 이 유효한지 여부. Identity 를 '미초기화' 센티넬로 쓰면
    // odom 원점에 정지한 정상 상태(= 출발 직전)와 구분되지 않는다.
