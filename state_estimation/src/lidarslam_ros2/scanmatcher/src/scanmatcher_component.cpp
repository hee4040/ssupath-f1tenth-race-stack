  ...
  broadcaster_(this)
{
  ...
  int vgicp_max_iterations;
  double vgicp_transformation_epsilon;
  double vgicp_rotation_epsilon;
  ...
  get_parameter("vgicp_resolution", vgicp_resolution_);
  declare_parameter("vgicp_k_correspondences", 20);
  get_parameter("vgicp_k_correspondences", vgicp_k_correspondences_);
  declare_parameter("vgicp_nn_method", "BRUTEFORCE");
  get_parameter("vgicp_nn_method", vgicp_nn_method_);
  declare_parameter("vgicp_kernel_width", 0.5);
  get_parameter("vgicp_kernel_width", vgicp_kernel_width_);
  declare_parameter("vgicp_max_iterations", 25);
  get_parameter("vgicp_max_iterations", vgicp_max_iterations);
  declare_parameter("vgicp_transformation_epsilon", 1e-3);
  get_parameter("vgicp_transformation_epsilon", vgicp_transformation_epsilon);
  declare_parameter("vgicp_rotation_epsilon", 2e-3);
  get_parameter("vgicp_rotation_epsilon", vgicp_rotation_epsilon);
  ...
  // z 밴드 필터 — 수평 측위에 기여하지 못하는 바닥/천장 제거.
  // 이 로비 맵은 점의 75.5%가 천장(z>=2.2), 6.8%가 바닥이고 벽/구조물은 17.6%뿐이다.
  // 스캔도 58.7%가 천장이라 정합 비용의 77%를 '수평 위치를 전혀 구속하지 않는' 수평면이
  // 차지한다(평면은 z/roll/pitch만 구속). 그 결과 종방향 비용함수가 거의 평평해져
  // 0728_1914 정지구간에서 NDT가 벽 기준 참값보다 0.89m +- 0.10m 앞으로 밀려 있었다.
  // 벽만(z 0.2~2.2) 쓰면 같은 프레임에서 골 깊이 8.3%p -> 28.7%p, 반치폭 1.30m -> 0.80m.
  // 스캔은 base_link, 맵은 map 프레임에 적용된다(평지에서 base_link z ~ -0.065라 차이 6.5cm).
  declare_parameter("enable_z_filter", false);
  get_parameter("enable_z_filter", enable_z_filter_);
  declare_parameter("scan_min_z", 0.2);
  get_parameter("scan_min_z", scan_min_z_);
  declare_parameter("scan_max_z", 2.2);
  get_parameter("scan_max_z", scan_max_z_);
  ...
  std::cout << "vgicp_resolution[m]:" << vgicp_resolution_ << std::endl;
  std::cout << "vgicp_k_correspondences:" << vgicp_k_correspondences_ << std::endl;
  std::cout << "vgicp_nn_method:" << vgicp_nn_method_ << std::endl;
  std::cout << "vgicp_kernel_width[m]:" << vgicp_kernel_width_ << std::endl;
  std::cout << "vgicp_max_iterations:" << vgicp_max_iterations << std::endl;
  std::cout << "vgicp_transformation_epsilon[m]:" << vgicp_transformation_epsilon << std::endl;
  std::cout << "vgicp_rotation_epsilon:" << vgicp_rotation_epsilon << std::endl;
  ...
  std::cout << "enable_z_filter:" << std::boolalpha << enable_z_filter_ <<
    " [" << scan_min_z_ << ", " << scan_max_z_ << "]" << std::endl;
  ...
  } else if (registration_method_ == "VGICP_CUDA") {
    boost::shared_ptr<fast_gicp::FastVGICPCuda<pcl::PointXYZI, pcl::PointXYZI>>
      vgicp(new fast_gicp::FastVGICPCuda<pcl::PointXYZI, pcl::PointXYZI>());
    vgicp->setResolution(vgicp_resolution_);
    vgicp->setCorrespondenceRandomness(vgicp_k_correspondences_);

    fast_gicp::NearestNeighborMethod nn =
      fast_gicp::NearestNeighborMethod::GPU_BRUTEFORCE;
    if (vgicp_nn_method_ == "RBF_KERNEL") {
      nn = fast_gicp::NearestNeighborMethod::GPU_RBF_KERNEL;
      vgicp->setKernelWidth(vgicp_kernel_width_);
    } else if (vgicp_nn_method_ == "CPU_KDTREE") {
      nn = fast_gicp::NearestNeighborMethod::CPU_PARALLEL_KDTREE;
    }
    vgicp->setNearestNeighborSearchMethod(nn);
    vgicp->setRegularizationMethod(fast_gicp::RegularizationMethod::PLANE);

    // 최악 지연 상한 (AGX Orin 이관 시 추가)
    // LsqRegistration 기본값은 max_iterations=64 / transformation_epsilon=5e-4(0.5mm).
    // 저특징·기하 aliasing 구간에서 수렴 판정이 안 서면 외부 64회 × LM 내부 최대 10스텝까지
    // GPU linearize가 반복돼 정합 지연이 폭증한다(0715 bag의 600~870ms 스파이크 양상과 일치).
    // 레이싱에선 "늦고 정확한 pose"보다 "제때 도착한 pose"가 낫다 — 대부분 프레임은
    // is_converged로 조기 종료되므로 평시 지연은 그대로고, 꼬리(p95/max)만 잘린다.
    // 상한에 걸려도 use_odom guess 기준 근처라 결과는 유효하며 EKF 지연게이트를 지킬 수 있다.
    // 0.5mm 수렴 임계는 라이다 노이즈(~cm)보다 과하게 빡빡해 반복만 늘리므로 1mm로 완화.
    vgicp->setMaximumIterations(vgicp_max_iterations);
    vgicp->setTransformationEpsilon(vgicp_transformation_epsilon);
    vgicp->setRotationEpsilon(vgicp_rotation_epsilon);
  ...
  // 측위용 맵은 스캔 수신과 무관하게 시작 시 즉시 로드/발행
  // (라이다가 꺼져 있어도 rviz에서 맵이 보이도록. 기존에는 첫 스캔 콜백에서 lazy 로드했음)
  {
    RCLCPP_INFO(get_logger(), "loading map: %s", map_path_.c_str());
    pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_ptr(new pcl::PointCloud<pcl::PointXYZI>());
    if (pcl::io::loadPCDFile(map_path_, *cloud_ptr) < 0 || cloud_ptr->points.empty()) {
      RCLCPP_ERROR(
        get_logger(), "failed to load map, localization disabled: %s", map_path_.c_str());
    } else {
      // 맵 target에도 스캔과 같은 z 밴드를 적용한다. 한쪽만 걸면 남은 쪽의 바닥/천장이
      // 대응점 없는 점으로 남아 비용만 키운다.
      if (enable_z_filter_) {
        pcl::PointCloud<pcl::PointXYZI>::Ptr band(new pcl::PointCloud<pcl::PointXYZI>());
        band->points.reserve(cloud_ptr->points.size());
        for (const auto & p : cloud_ptr->points) {
          if (scan_min_z_ < p.z && p.z < scan_max_z_) {band->points.push_back(p);}
        }
        band->width = band->points.size();
        band->height = 1;
        RCLCPP_INFO(
          get_logger(), "맵 z 필터 [%.2f, %.2f]: %zu -> %zu점",
          scan_min_z_, scan_max_z_, cloud_ptr->points.size(), band->points.size());
        cloud_ptr.swap(band);
      }
      // 맵 다운샘플 (옵션)
      ...
      //  (시각화용) map을 map 프레임으로 latched 퍼블리시
...
void ScanMatcherComponent::initializePubSub()
{
  ...
  if (enable_angle_filter_ || use_min_max_filter_ || enable_z_filter_) {
    ...
    const bool use_z = enable_z_filter_;
    ...
    for (const auto & p : tmp_ptr->points) {
      ...
      // 바닥/천장 제거 (base_link 기준 높이)
      if (use_z && keep) {
        if (!(scan_min_z_ < p.z && p.z < scan_max_z_)) keep = false;
      }
  ...
  initialpose_rviz_sub_ =
    create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
      ...
      [this](const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg)
      {
        ...
        // (다음 스캔에서 델타 없이 시작하고, 그 프레임의 odom을 새 기준으로 잡는다)
  ...
  map_pub_options.use_intra_process_comm = rclcpp::IntraProcessSetting::Disable;
  ...
  const rclcpp::Time stamp)
{
  ...
  if (use_odom_) {
    try {
      const geometry_msgs::msg::TransformStamped odom_trans = tfbuffer_.lookupTransform(
        odom_frame_id_, robot_frame_id_, tf2_ros::fromMsg(stamp));
      const Eigen::Affine3d odom_affine = tf2::transformToEigen(odom_trans);
      const Eigen::Matrix4f odom_mat = odom_affine.matrix().cast<float>();
      // '이전 odom 있음'을 previous_odom_mat_ != Identity 로 판정하면 안 된다.
      // Identity는 '미초기화'가 아니라 '차가 odom 원점에 정지해 있음'이라는 정당한 값이고,
      // 그게 바로 매 주행의 출발 직전 상태다. 그래서 예전 코드는 odom 델타가 가장 필요한
      // 출발 순간에 이를 버렸고, /ndt_pose가 3프레임(0.3s)간 얼어붙어 회전 약 5°를 통째로
      // 놓쳤다(0723_1606 bag). VGICP는 국소 최적화라 그렇게 벌어진 오차를 되돌리지 못해
      // 이후 주행 내내 yaw 오차를 안고 갔다. 명시적 플래그로 분리한다.
      ...
      // 조회 실패 시 identity를 델타에 쓰면 guess가 크게 점프하므로
      // 이번 스캔은 델타 없이(이전 pose 그대로) 정합하고 누적자만 무효화
      RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 5000,
        "odom->base_link TF 조회 실패(첫 주행 입력 전이면 정상): %s", e.what());
      has_previous_odom_ = false;
    }
  }
  ...
  const Eigen::Matrix4f final_transformation, const rclcpp::Time stamp)
{
  ...
  if(publish_tf_){
    ...
    bool tf_sent = false;
    if (use_odom_) {
      // use_odom 시 vesc_to_odom이 odom->base_link를 발행하므로 여기서
      // base_link를 또 붙이면 부모가 둘이 됨. REP-105대로 map->odom을 발행.
      try {
        const geometry_msgs::msg::TransformStamped odom_trans = tfbuffer_.lookupTransform(
          odom_frame_id_, robot_frame_id_, tf2::TimePointZero);
        Eigen::Isometry3d T_map_base = Eigen::Isometry3d::Identity();
        T_map_base.linear() = rot_mat;
        T_map_base.translation() = position;
        const Eigen::Isometry3d T_odom_base = tf2::transformToEigen(odom_trans);
        const Eigen::Isometry3d T_map_odom = T_map_base * T_odom_base.inverse();
        transform_stamped.child_frame_id = odom_frame_id_;
        transform_stamped.transform = tf2::eigenToTransform(T_map_odom).transform;
        broadcaster_.sendTransform(transform_stamped);
        tf_sent = true;
      } catch (tf2::TransformException & e) {
        RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 5000,
          "odom->base_link TF 없음, map->base_link로 폴백: %s", e.what());
      }
    }
    if (!tf_sent) {
