...
void SlamComponent::initializePubSub()
{
...
void SlamComponent::initializeMap(const pcl::PointCloud <pcl::PointXYZI>::Ptr & tmp_ptr, const std_msgs::msg::Header & header)
{
...
void SlamComponent::receiveCloud(
...
void SlamComponent::publishMapAndPose(
  const pcl::PointCloud<pcl::PointXYZI>::ConstPtr & cloud_ptr,
  const Eigen::Matrix4f final_transformation, const rclcpp::Time stamp)
{
  ...
  if (trans_ >= trans_for_mapupdate_ && !mapping_flag_) {
    ...
    mapping_task_ =
      ...
      std::bind(
        &SlamComponent::updateMap, this, cloud_ptr,
...
void SlamComponent::receiveImu(const sensor_msgs::msg::Imu msg)
{
  if (!use_imu_) {return;}
...
void SlamComponent::publishMap(const lidarslam_msgs::msg::MapArray & map_array_msg , const std::string & map_frame_id)
{
...
geometry_msgs::msg::TransformStamped SlamComponent::calculateMaptoOdomTransform(
...
RCLCPP_COMPONENTS_REGISTER_NODE(graphslam::SlamComponent)
