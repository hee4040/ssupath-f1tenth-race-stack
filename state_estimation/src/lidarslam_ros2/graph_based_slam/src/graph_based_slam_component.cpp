...
void GraphBasedSlamComponent::searchLoop()
{
  ...
  if (is_candidate) {
    ...
    for (int j = 0; j <= 2 * search_submap_num_; ++j) {
      const int submap_id = id_min + j - search_submap_num_;
      if (submap_id < 0 || submap_id >= num_submaps) {continue;}
      auto near_submap = map_array_msg.submaps[submap_id];
  ...
  bool do_save_map)
{
  ...
  if (do_save_map) {
    const int ret = pcl::io::savePCDFileBinary(map_path_, *map_ptr);
    if (ret == 0) {
      RCLCPP_INFO(get_logger(), "saved map to %s, number of points: %ld",
        map_path_.c_str(), map_ptr->size());
    } else {
      RCLCPP_ERROR(get_logger(), "failed to save map to %s", map_path_.c_str());
    }
  }
