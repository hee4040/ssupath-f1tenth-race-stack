...
class StateMachineParams:
    def __init__(self, node: StateMachine) -> None:
        ...
        # trailing_to_gbtrack_counting_threshold 파라미터 추가
        descriptor = ParameterDescriptor(
            description="Number of consecutive gb_free detections before switching from TRAILING to GB_TRACK\n",
            type=ParameterType.PARAMETER_INTEGER,
        ...
        # 회피 궤적 속도 상한용 접지 한계 (states._fuse_speed_from_global 에서 사용)
        descriptor = ParameterDescriptor(
            description="Max lateral acceleration used to cap avoidance trajectory speed in m/s^2\n",
            read_only=False,
            type=ParameterType.PARAMETER_DOUBLE,
            floating_point_range=[FloatingPointRange(from_value=1.0, to_value=15.0, step=0.1)]
        )
        node.set_descriptor("ay_max_mps2", descriptor=descriptor)
        self.ay_max_mps2: float = node.get_parameter("ay_max_mps2").value

        descriptor = ParameterDescriptor(
            description="Max longitudinal deceleration used to pre-brake into an avoidance in m/s^2\n",
            read_only=False,
            type=ParameterType.PARAMETER_DOUBLE,
            floating_point_range=[FloatingPointRange(from_value=1.0, to_value=15.0, step=0.1)]
        )
        node.set_descriptor("ax_max_mps2", descriptor=descriptor)
        self.ax_max_mps2: float = node.get_parameter("ax_max_mps2").value

        # ---- 고속 회피 감속 강화 --------------------------------------------
        descriptor = ParameterDescriptor(
            description="Speed below which no extra high-speed braking is applied in m/s\n",
            read_only=False,
            type=ParameterType.PARAMETER_DOUBLE,
            floating_point_range=[FloatingPointRange(from_value=0.0, to_value=10.0, step=0.1)]
        )
        node.set_descriptor("hs_brake_v_lo", descriptor=descriptor)
        self.hs_brake_v_lo: float = node.get_parameter("hs_brake_v_lo").value

        descriptor = ParameterDescriptor(
            description="Speed at and above which the full high-speed braking factor applies in m/s\n",
            read_only=False,
            type=ParameterType.PARAMETER_DOUBLE,
            floating_point_range=[FloatingPointRange(from_value=0.0, to_value=15.0, step=0.1)]
        )
        node.set_descriptor("hs_brake_v_hi", descriptor=descriptor)
        self.hs_brake_v_hi: float = node.get_parameter("hs_brake_v_hi").value

        descriptor = ParameterDescriptor(
            description="Extra speed scaling applied to the avoidance trajectory at high speed\n"
                        "1.0 disables the feature\n",
            read_only=False,
            type=ParameterType.PARAMETER_DOUBLE,
            floating_point_range=[FloatingPointRange(from_value=0.2, to_value=1.0, step=0.05)]
        )
