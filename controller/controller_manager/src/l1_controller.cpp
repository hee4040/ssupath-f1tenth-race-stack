...
#include "f110_msgs/msg/l1controller_control.hpp"
...
public:
          ...
          .automatically_declare_parameters_from_overrides(true))
  {
    ...
    l1_param_path_ = this->get_parameter("l1_controller_param_path").as_string();
    ...
    lookahead_pub_  = create_publisher<visualization_msgs::msg::Marker>("lookahead_point", 1);
    trailing_pub_   = create_publisher<visualization_msgs::msg::Marker>("trailing_opponent_marker", 1);
...
private:
  ...
  T get_remote_parameter(const std::string &remote_node, const std::string &name) {
    ...
    // Jetson 에서 base_system 이 map.pcd 를 로딩하는 동안에는 원격 파라미터 서비스의
    // 등장/왕복이 수 초를 넘길 수 있다.
    // 예전 구현은 5 초 타임아웃 후 곧바로 throw 했고, 이 예외가 생성자에서 잡히지 않아
    // 노드가 SIGABRT(-6) 로 죽었다. 그러면 /l1controller/control 이 없어 /drive 가 비고
    // **차가 아예 출발하지 않는다**. (base_system 을 띄운 뒤 30~40 초만 기다리고
    //  head_to_head 런치를 띄우면 재현된다)
    // 따라서 여유 있는 총 예산 안에서 인내심 있게 재시도한다.
    const auto deadline = std::chrono::steady_clock::now() + std::chrono::seconds(120);
    std::shared_ptr<Service::Response> res;
    while (rclcpp::ok() && std::chrono::steady_clock::now() < deadline) {
      if (!client->wait_for_service(std::chrono::seconds(2))) {
        RCLCPP_WARN(get_logger(), "waiting for remote parameter service %s ...", service_name.c_str());
        continue;
      }
      ...
      RCLCPP_WARN(get_logger(), "retrying parameter '%s' from %s ...", name.c_str(), remote_node.c_str());
      client->remove_pending_request(future);
    }

    if (!res) {
      throw std::runtime_error("Timeout getting parameter '" + name + "' from " + remote_node);
    }
    if (res->values.empty()) {
  ...
  void init_map_controller() {
    ...
    map_controller_ = std::make_unique<MAP_Controller>(
      ...
      l1_params_["curvature_scale"].as<double>(),
      l1_params_["ok_thresh"].as<double>(),
      l1_params_["hard_thresh"].as<double>(),
      l1_params_["min_scale"].as<double>(),
      l1_params_["steer_rate_thresh"].as<double>(),
      rate_hz_,
      LUT_name_,
  ...
  void load_l1_params_from_yaml_() {
    // launch에서 넘겨준 경로 사용
    const std::string cfg = l1_param_path_;
    RCLCPP_INFO(get_logger(), "Loading L1 params from: %s", cfg.c_str());

    YAML::Node y = YAML::LoadFile(cfg);
    const auto node = y["l1_controller"]["ros__parameters"];
    ...
    for (const auto &k : keys) {
      if (!node[k]) {
        RCLCPP_WARN(get_logger(), "Key '%s' not found in L1 param file", k.c_str());
        continue;
      }
      l1_params_[k] = node[k];
    }
  ...
  void declare_update_params_and_watch_() {
    ...
    decl("curvature_scale", l1_params_["curvature_scale"], param_desc(fp_range(0.0,1.5,0.01)));
    decl("ok_thresh", l1_params_["ok_thresh"], param_desc(fp_range(0.0,90.0,0.1)));
    decl("hard_thresh", l1_params_["hard_thresh"], param_desc(fp_range(0.0,90.0,0.1)));
    decl("min_scale", l1_params_["min_scale"], param_desc(fp_range(0.0,1.0,0.01)));
    decl("steer_rate_thresh", l1_params_["steer_rate_thresh"], param_desc(fp_range(0.0,1.0,0.01)));
  ...
  void on_parameter_event_(const rcl_interfaces::msg::ParameterEvent::SharedPtr event) {
    ...
    if (mode_ == "MAP" && map_controller_) {
      ...
      map_controller_->curvature_scale          = get_parameter("curvature_scale").as_double();
      map_controller_->ok_thresh                = get_parameter("ok_thresh").as_double();
      map_controller_->hard_thresh              = get_parameter("hard_thresh").as_double();
      map_controller_->min_scale                = get_parameter("min_scale").as_double();
      map_controller_->steer_rate_thresh        = get_parameter("steer_rate_thresh").as_double();
      ...
      pp_controller_->curvature_scale           = get_parameter("curvature_scale").as_double();
      pp_controller_->ok_thresh                = get_parameter("ok_thresh").as_double();
      pp_controller_->hard_thresh              = get_parameter("hard_thresh").as_double();
      pp_controller_->min_scale                = get_parameter("min_scale").as_double();
      pp_controller_->steer_rate_thresh        = get_parameter("steer_rate_thresh").as_double();
    }
  ...
  void control_loop() {
    ...
    f110_msgs::msg::L1controllerControl msg;
...
private:
  ...
  rclcpp::Publisher<f110_msgs::msg::L1controllerControl>::SharedPtr drive_pub_;
...
int main(int argc, char **argv) {
  ...
  // 생성자에서 던진 예외가 그대로 올라오면 std::terminate -> SIGABRT(-6) 가 되어
  // launch 로그에 "process has died ... exit code -6" 만 남고 원인을 알 수 없다.
  // 원인을 로그로 남기고 깨끗하게 종료한다.
  try {
    rclcpp::spin(std::make_shared<Controller>());
  } catch (const std::exception &e) {
    RCLCPP_FATAL(rclcpp::get_logger("l1_controller"), "fatal during startup: %s", e.what());
    rclcpp::shutdown();
    return 1;
  }
  rclcpp::shutdown();
